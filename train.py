...
def fit_ont_epoch(net,epoch,epoch_size_train,epoch_size_val,train_dataloader,val_dataloader,Epoch,cuda):
    ...
    torch.save(model.state_dict(),  args.train_weight_dir+'train_model_weight.pth')
...
if __name__ == "__main__":
    ...
    # 训练的冻结的epoch数
    parser.add_argument('--f_epochs', default=20, type=int, metavar='N',
                        help='number of total epochs to run')
    # 训练的解冻的epoch数
    parser.add_argument('--uf_epochs', default=20, type=int, metavar='N',
                        help='number of total epochs to run')
    # 训练的冻结的batch size
    parser.add_argument('--f_batch_size', default=1, type=int, metavar='N',
                        help='batch size when training.')
    # 训练的解冻的batch size
    parser.add_argument('--uf_batch_size', default=1, type=int, metavar='N',
                        help='batch size when training.')
    # 设置冻结的学习率
    ...
    train_dataset   = VOC2012DataSet(voc_root=args.data_path, txt_name='try.txt')
    val_dataset     = VOC2012DataSet(voc_root=args.data_path, txt_name='try.txt')
