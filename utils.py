...
class AnchorTargetCreator(object):
    ...
    def __call__(self, bbox, anchor, img_size):
        ...
        #   初始化的时候全部设置为-1
        # ------------------------------------------ #
        gt_anchor_binary = np.empty((len(anchor),), dtype=np.int32)
        gt_anchor_binary.fill(-1)
        ################################################
        #----------------------------------------------#
        #   anchor和bbox的iou
        ...
        gt_anchor_bbox = bbox[argmax_ious]
        #---------------------------------------------------------#
        ...
        #   每个真实框至少对应一个先验框
        # ----------------------------------------------------- #
        gt_anchor_binary[max_ious < self.neg_iou_thresh] = 0
        gt_anchor_binary[max_ious >= self.pos_iou_thresh] = 1
        if len(gt_argmax_ious)>0:
            gt_anchor_binary[gt_argmax_ious] = 1        
        # ----------------------------------------------------- #
        #   判断正样本数量是否大于128，如果大于则限制在128
        # ----------------------------------------------------- #
        n_pos = int(self.pos_ratio * self.n_sample)
        pos_index = np.where(gt_anchor_binary == 1)[0]
        if len(pos_index) > n_pos:
            disable_index = np.random.choice(pos_index, size=(len(pos_index) - n_pos), replace=False)
            gt_anchor_binary[disable_index] = -1
        # ----------------------------------------------------- #
        #   平衡正负样本，保持总数量为256
        # ----------------------------------------------------- #
        n_neg = self.n_sample - np.sum(gt_anchor_binary == 1)
        neg_index = np.where(gt_anchor_binary == 0)[0]
        if len(neg_index) > n_neg:
            disable_index = np.random.choice(neg_index, size=(len(neg_index) - n_neg), replace=False)
            gt_anchor_binary[disable_index] = -1
   
        if (gt_anchor_binary <= 0).all():
            raise ValueError('no label')
        gt_anchor_loc = bbox2loc(anchor, gt_anchor_bbox)

        return gt_anchor_loc, gt_anchor_binary
...
class ProposalTargetCreator(object):
    def __init__(self, n_sample=128, pos_ratio=0.5, pos_iou_thresh=0.5, neg_iou_thresh_high=0.5, neg_iou_thresh_low=0):
        self.n_sample = n_sample
        self.pos_ratio = pos_ratio
        self.pos_prosal_per_image = np.round(self.n_sample * self.pos_ratio)
        self.pos_iou_thresh = pos_iou_thresh
        self.neg_iou_thresh_high = neg_iou_thresh_high
        self.neg_iou_thresh_low = neg_iou_thresh_low

    def __call__(self, prosal, bbox, label, loc_normalize_mean=(0., 0., 0., 0.), loc_normalize_std=(0.1, 0.1, 0.2, 0.2)):
        if len(bbox)==0:
                    raise ValueError('no bbox')
      
        prosal = np.concatenate((prosal.detach().cpu().numpy(), bbox), axis=0)
        # ----------------------------------------------------- #
        #   计算建议框和真实框的重合程度
        # ----------------------------------------------------- #
        iou = bbox_iou(prosal, bbox)
        #---------------------------------------------------------#
        ...
        #   真实框的标签要+1因为有背景的存在
        #---------------------------------------------------------#
        gt_prosal_label = label[gt_assignment] + 1
        #----------------------------------------------------------------#
        #   满足建议框和真实框重合程度大于neg_iou_thresh_high的作为正样本
        #   将正样本的数量限制在self.pos_roi_per_image以内
        #----------------------------------------------------------------#
        pos_index = np.where(max_iou >= self.pos_iou_thresh)[0]
        pos_prosal_per_this_image = int(min(self.pos_prosal_per_image, pos_index.size))
        if pos_index.size > 0:
            pos_index = np.random.choice(pos_index, size=pos_prosal_per_this_image, replace=False)
        ...
        neg_roi_per_this_image = self.n_sample - pos_prosal_per_this_image
        ...
        sample_prosal = prosal[keep_index]

        gt_prosal_loc = bbox2loc(sample_prosal, bbox[gt_assignment[keep_index]])
        gt_prosal_loc = ((gt_prosal_loc - np.array(loc_normalize_mean, np.float32)) / np.array(loc_normalize_std, np.float32))

        gt_prosal_label = gt_prosal_label[keep_index]
        gt_prosal_label[pos_prosal_per_this_image:] = 0

        return sample_prosal, gt_prosal_loc, gt_prosal_label
